  ...
  return (<Container maxW={'container.xl'}>
    {
        loading?<Loader />:(
          ...
          <VStack spacing={'4'} p={'16'} alignItems={'flex-start'}>
            <Text fontSize={'small'} alignSelf={'center'} opacity={'0.7'}>Last Updatted On: {Date(coin.market_data.last_updated).split('G')[0]}</Text>
            ...
            <Badge fontSize={'2xl'} bgColor={'grey'} color={'white'}>{`#${coin.market_cap_rank}`}</Badge>
...
const Item=({title,value}) => (
  <HStack justifyContent={'space-between'} w={'full'} my={'4'}>
    <Text fontFamily={"Bebas Nueue"} letterSpacing={'widest'}>{title}</Text>
    <Text>{value}</Text>
  </HStack>
)
